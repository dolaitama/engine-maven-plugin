...
public class GenerateUtil {
	...
	public static void product(Map<String, String> map) throws Exception{
		...
		for (Map.Entry<String, String> entry : map.entrySet()) {
			...
			Constant.preTable = preTable;
			Constant.annotation = entry.getValue();
			Constant.tableName = key;
			//velocity模板参数设置
			if(StringUtils.isNotBlank(preTable)){
				Constant.ctx.put("pretable", Constant.preTable);//库名
			}else{
				Constant.ctx.put("pretable", null);//库名
			}
...
//		}
	...
	private static void initMySql(String key, Connection conn) throws Exception {
		String schema = Constant.database;
		if(StringUtils.isNotBlank(Constant.preTable)){
			schema = Constant.preTable;
		}
		String sql = "SELECT COLUMN_NAME,DATA_TYPE,COLUMN_COMMENT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '"
				+ key + "' AND TABLE_SCHEMA = '" + schema + "'";
